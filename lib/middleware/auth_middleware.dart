...
class AuthMiddleware extends GetMiddleware {
  ...
  RouteSettings? redirect(String? route) {
    try {
      ...
      // AuthController가 아직 초기화되지 않은 경우 로그인 화면으로 이동
      print('AuthMiddleware - AuthController 초기화 중: $e');
      return const RouteSettings(name: AppRoutes.login);
    }
...
class GuestMiddleware extends GetMiddleware {
  ...
  RouteSettings? redirect(String? route) {
    try {
      ...
      // AuthController가 아직 초기화되지 않은 경우 현재 화면 유지
      print('GuestMiddleware - AuthController 초기화 중: $e');
      return null;
    }
...
class SessionMiddleware extends GetMiddleware {
  ...
  RouteSettings? redirect(String? route) {
    ...
    try {
      ...
      // AuthController가 아직 초기화되지 않은 경우 로그인 화면으로 이동
      print('AuthController 초기화 중 - 로그인 화면으로 이동: $e');
      return const RouteSettings(name: AppRoutes.login);
    }
